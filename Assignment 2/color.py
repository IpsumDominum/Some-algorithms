  ...
  for op in pwTokens:
    ...
    if o.isEdgeOp():
      ...
      for i in range(3**(t+1)): # update all state boundary combinations
        C=unrankColoring(i)
        if C[v1]==C[v2]:
          state[i]=10000000 # edge makes invalid coloring
    else: # isVertOp()           
      v1=o.v1()
      for i in range(3**(t+1)):
        ...
        if C[v1]==0: # process all coloring states only when v1 is colored 0
          ...
          min_c = min(state[i],state[i1],state[i2])
          #If at least one combo is valid
          if(min_c!=10000000):
              state[i] = min_c+1
              state[i1] = min_c+2
              state[i2] = min_c+3
...
def tw3Col(G):
  ...
  G=G.strip(); # print("G=",G)
  state=array.array('i', map((lambda n:sum(unrankColoring(n))+t+1), range(3**(t+1))))
  ...
  for i in range(1,len(G)): # doing a circle plus operator
    ...
    if lev==0:
      ...
      while True:
         k = G[i+1:len(G)].find('(')
         if k==-1: return pw3Col(re.findall('\d+',G[i+1:len(G)]),state1)
         for j in range(i+1+k,len(G)):
             ...
             if lev==0:
                 ...
                 for l in range(3**(t+1)): # now update state for circle plus
                     min_c_appended = state1[l]+state2[l] - (sum(unrankColoring(l))+t+1)
                     state.append(min_c_appended)
                 if j+1 < len(G):
                     state1=state
                     i=j+1
...
while True:
    ...
    if(min_c>=10000000):
      ...
      print(min_c)
    #print("3-Colorable =", ("No","Yes")[max(tw3Col(s))], "\n")
    if t==0: break